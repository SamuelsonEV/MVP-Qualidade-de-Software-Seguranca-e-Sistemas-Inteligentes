...
*/
const getList = async () => {
  let url = 'http://127.0.0.1:5000/arrozs';
  fetch(url, {
    method: 'get',
  })
    .then((response) => response.json())
    .then((data) => {
      data.arrozs.forEach(item => insertList(item.name, 
                                                item.preg, 
                                                item.plas,
                                                item.pres,
                                                item.skin,
...
const postItem = async (inputPatient, inputPreg, inputPlas,
  ...
  let url = 'http://127.0.0.1:5000/arroz';
  fetch(url, {
    method: 'post',
    body: formData
  })
...
const deleteItem = (item) => {
  console.log(item)
  let url = 'http://127.0.0.1:5000/arroz?name='+item;
  fetch(url, {
    method: 'delete'
  })
    .then((response) => response.json())
...
const newItem = async () => {
  ...
  const checkUrl = `http://127.0.0.1:5000/arrozs?nome=${inputPatient}`;
  fetch(checkUrl, {
    method: 'get'
  })
    .then((response) => response.json())
    .then((data) => {
      if (data.arrozs && data.arrozs.some(item => item.name === inputPatient)) {
        alert("O arroz já está cadastrado.\nCadastre o ID-TEST com um nome diferente ou atualize o existente.");
      } else if (inputPatient === '') {
        alert("O nome do arroz não pode ser vazio!");
...
const insertList = (namePatient, preg, plas,pres, skin, test, mass, pedi, age, outcome) => {
  var item = [namePatient, preg, plas,pres, skin, test, mass, pedi, outcome];
  var table = document.getElementById('myTable');
